...
def insbokred (cur, fileid, mjd, fil, filename) :

    baselist = {'u':'/data/red/bok/u/pass1/', 'v':'/data/red/bok/v/pass1/'}
    if not baselist.has_key(fil) : return (0)
    basepath = baselist[fil]

    filepart = filename.split('/')
    basename = filepart[8][0:10]
    caldate = '%4.4d%2.2d%2.2d' % lzju.cal(mjd)[0:3]

    redpath = basepath+filepart[5]+'/'+caldate+'/'+filepart[7]+'/'+basename+'/'
    dbfile = redpath+basename+'.db.txt'
    if not os.path.exists(dbfile) : return (0)
...
if __name__ == '__main__' :
    ...
    cur.execute("SELECT FileID, MJD, FilterCode, FileName FROM ObsFileBasic " + \
        "WHERE Telescope='B' AND (Type='S' OR Type='O') ORDER BY FileID")
    tb = cur.fetchall()
    cc=0
    for row in tb :
        ls=insbokred (cur, row[0], row[1], row[2], row[3])
        cc += (1 if ls > 0 else 0)
        db.commit()
        #if cc == 50 : break
