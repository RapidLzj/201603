pro zb_pp_flat, listfile, bias, outfile
    ...
    flat_dat = dblarr(nx, ny, namp, n_file)
    bias_dat = dblarr(nx, ny, namp)
    flat_1 = dblarr(nx, ny, namp)

    print, bias, format='("Load BIAS : ",A / 12x,"Section : ",$)'
    for gg = 1, namp do begin
        print, gg, format='(" ",I2,",",$)'
        bias_dat[*,*, gg-1] = mrdfits(bias, gg, /silent, /dscale)
    endfor
    print, ''
    ...
    for ff = 0, n_file-1 do begin
        ...
        print, ' Normalize ...'
        flat_1 -= bias_dat
        flat_1 /= median(flat_1, /double)
        flat_dat[*,*,*, ff] = flat_1
    endfor

    print, format='("Merging....")'
    merge_flat_dat = median(flat_dat, dim=4, /even, /double)

    for gg = 1, namp do begin
        hdr1 = headfits(flatfiles[0], ext=gg, /silent)
        ;sxaddpar, hdr1, 'BZERO', 0.0
        mwrfits, float(merge_flat_dat[*,*, gg-1]), outfile, hdr1, /silent
    endfor
