function zn_pip_flagtitle, flag
    @zh_const.pro
...
pro zn_pip_report, sci_path, file, $
    ...
    oldradeg      = sxpar(hdr, 'OLD-CRV1')
    olddec        = sxpar(hdr, 'OLD-DEC' )
    olddecdeg     = sxpar(hdr, 'OLD-CRV2')
    ...
    printf, lun_out, format='(5(A10,":",A5,2x))', $
            'BIAS&FLAT', zn_pip_flagtitle(flag_bf),  $
            'SOURCEAPER', zn_pip_flagtitle(flag_sex), $
            'ASTROMETRY ', zn_pip_flagtitle(flag_wcs), $
            'FLUXCALIBR', zn_pip_flagtitle(flag_mag), $
            'CROSSIDENT', zn_pip_flagtitle(flag_cross)
