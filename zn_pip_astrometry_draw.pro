...
gx = fltarr(41,41) & gy = gx
for ii= 0,40 do gx[ii,*]=(ii-20)*100
for ii= 0,40 do gy[*,ii]=(ii-20)*100
zh_xy2ad, gx, gy, result, rag, decg
; x/y cross
xcrossx = (5 - findgen(11)) * 50 & xcrossy = fltarr(11)
...
oplot, [8,25], [0,0]+med_resid_ra+sig_resid_ra, line=1
oplot, [8,25], [0,0]+med_resid_ra-sig_resid_ra, line=1
oplot, [8,25], [0,0]+med_resid_ra, line=2
...
oplot, [8,25], [0,0]+med_resid_dec+sig_resid_dec, line=1
oplot, [8,25], [0,0]+med_resid_dec-sig_resid_dec, line=1
oplot, [8,25], [0,0]+med_resid_dec, line=2

plot, crxs[sid], resid_ra, psym=2, symsize=0.1, xrange=[-2000,2000], $
  xtitle='X to center (pixel)', ytitle='Resid RA (arcsec)', yr=[-2,2], $ ;title='Resid RA VS X',
  pos=[0.05, 0.30, 0.55, 0.50]
oplot, [-2000,2000], [0,0]+med_resid_ra+sig_resid_ra, line=1
oplot, [-2000,2000], [0,0]+med_resid_ra-sig_resid_ra, line=1
oplot, [-2000,2000], [0,0]+med_resid_ra, line=2

plot, crys[sid], resid_dec, psym=2, symsize=0.1, xrange=[-2000,2000], $
  xtitle='Y to center (pixel)', ytitle='Resid Dec (arcsec)', yr=[-2,2], $ ;title='Resid Dec VS Y',
  pos=[0.05, 0.05, 0.55, 0.25]
oplot, [-2000,2000], [0,0]+med_resid_dec+sig_resid_dec, line=1
oplot, [-2000,2000], [0,0]+med_resid_dec-sig_resid_dec, line=1
oplot, [-2000,2000], [0,0]+med_resid_dec, line=2
