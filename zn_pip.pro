pro zn_pip, rawpath, redpath, file, bias, flat, $
    ...
    if n_params() lt 3 then begin
        ...
        'NOWT image data process pipeline', $
        'Syntax:', $
        '  zn_onsite, rawpath, redpath, file, bias, flat, $', $
    ...
    else if ~keyword_set(catawcs) then catawcs = 'ub1'
    ; center keyword
    r_default, recenter
    ...
    ;ggs = string(indgen(16)+1,format='(I2.2)')  ; for BOK
    ggs = string(indgen(4)+1,format='(I1.1)')    ; for NOWT
    ...
    if ~ nobf then begin
        flag_bf = zn_pip_biasflat( rawpath, sci_path, file, bias, flat, $
    ...
    if ~ nosex and flag_bf ne FLAG_FAIL and flag_bf ne FLAG_ERROR then begin
        flag_sex = zn_pip_photometry( rawpath, sci_path, file, $
    ...
    if ~ nowcs and flag_sex ne FLAG_FAIL and flag_sex ne FLAG_ERROR then begin
        flag_wcs = zn_pip_astrometry( sci_path, file,  $
    ...
    if ~ nomag and flag_wcs ne FLAG_FAIL and flag_wcs ne FLAG_ERROR then begin
        flag_mag = zn_pip_magcalibrate( sci_path, file, $
    ...
    if cross and flag_wcs ne FLAG_FAIL and flag_wcs ne FLAG_ERROR then begin
        flag_cross = zn_pip_cross( sci_path, file, $
