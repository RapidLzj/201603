function zn_pip_astrometry, sci_path, file,  $
    ...
    ctra  = sxpar(hdr, 'CRVAL1')
    ctdec = sxpar(hdr, 'CRVAL2')
    ...
    ; rotate stars according to the rotater's angle
    ang = round(sxpar(hdr, 'ROTE-ANG') / 90.0) mod 4
    xi = stars.x & yi = stars.y
    if ang eq 0 then begin
      stars.x = yi         &  stars.y = xi
    endif else if ang eq 2 then begin
      stars.x = nx2-1 - yi & stars.y = ny2-1 - xi
    endif else if ang eq 1 then begin
      stars.x = nx2-1 - xi & stars.y = yi
    endif else if ang eq 3 then begin
      stars.x = xi         & stars.y = ny2-1 - yi
    endif

    if ang eq 0 or ang eq 2 then begin
      ; for ang 0/2, exchange x/y, so center will also be exchanged
      t = ctx & ctx = cty & cty = t
      ; update header
    ...
    ix = where(mindis gt 30.0d, n_bstar)
    xs = xs[ix] & ys = ys[ix] & mags = mags[ix] & magse = magse[ix] & pix = pix[ix]
    ...
    amaxdis = [0, 60.0d, 30.0d, 15d, 5.0d] / 3600.0d
    amatchlimit = [0, 10, 20, 30, 40]
    matchlimitcheck = 20

    openw, lun_out, sci_path + file + '.regress.txt', /get_lun
    ...
    while not finish1 do begin ;rep_n le max_rep_n and rep_n ge 0 do begin
        ...
        printf, lun_out, rep_n, nmatch, sigma_resid, $
    ...
    if fail1 or crxym gt 3500  then begin
        printf, lun_out, nmatch, crxym, format='("FAIL: nmatch = ",I3,"  crxym = ",F9.3)'
    ...
    printf, lun_out, result.aconst, result.bconst, r_hms([result.aconst/15, result.bconst]), $
        cdelt1*3600.0, cdelt2*3600.0, crota1, crota2, $
        format='("CENTER",2(1X,F10.6),2(1X,A12) , "  CDELT",2(1X,F7.4) , "  CROTA:",2(1X,F5.2))'
    printf, lun_out, crxm, crym, ((abs(crxm)>abs(crym)) lt 800)?'OK':'DOUBT', $
